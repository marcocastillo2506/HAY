  ...
  getCohortProfiles() {
    $.ajax({
      ...
      success: function(data) {
        this.setState({mentors: data[0], students: data[1]})
      }.bind(this),
  ...
  render() {
    return (<div>
      ...
      <div id='fondoBannerMatrix' className="cohort-content">
        <Switch>
          <Route exact path={this.props.match.path} render={(props) => <CohortLayout {...props} data={this.state}/>}/>
          <Route path={`${this.props.match.path}/:id`} render={(props) => <Profile {...props} data={this.state}/>}/>
        </Switch>
      </div>
    </div>)
  }
